...
/** 纵向布局容器。 */
public final class ColumnNode extends ContainerNode {

    public ColumnNode(NodeStyle style) {
        super(style);
    }
    ...
    public void measure(MeasureContext context) {
        ...
        for (UiNode child : kids) {
            child.measure(context.withAvailable(contentW, Math.max(0, contentH - usedMain)));
            if (!child.visibleNow()) {
                continue;
            }
            usedMain += child.height();
        ...
        height = resolvedH >= 0 ? resolvedH : pad.vertical() + usedMain;
    }
    ...
    public void layout(float x, float y) {
        ...
        float free = Math.max(0, contentH - total);
        ...
        float cursorY = contentY + offset;
        for (UiNode child : kids) {
            if (!child.visibleNow()) {
                continue;
            }
            if (align == CrossAlign.STRETCH && child.style().width().isAuto()) {
                child.overrideWidth(contentW);
            }
            float childW = child.width();
            float childX = switch (align) {
                case START, STRETCH -> contentX;
                case CENTER -> contentX + (contentW - childW) / 2;
                case END -> contentX + (contentW - childW);
            };
            child.layout(childX, cursorY);
            cursorY += child.height() + gap() + extraGap;
        }
    }
}
